...
const Index = () => {
  ...
  const filteredStudents = useMemo(() => {
    if (!query.trim()) return studentStatusMap;
    const q = query.toLowerCase();
    return studentStatusMap.filter(({ student }) => student.name.toLowerCase().includes(q));
  }, [studentStatusMap, query]);
  ...
  const quickActions = [
    ...
    {
      icon: Sparkles,
      label: isAr ? "عرض تجريبي" : "Demo Tour",
      desc: isAr ? "قدّم تجربة للمديرات" : "Showcase to principals",
  ...
  const openWhatsApp = useCallback((defaultText: string) => {
    const custom = window.prompt(isAr ? "عدّل الرسالة قبل الإرسال" : "Edit the message before sending", defaultText) ?? defaultText;
    const url = `https://wa.me/?text=${encodeURIComponent(custom)}`;
  ...
  const StatsSkeleton = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
      {[...Array(4)].map((_, i) => (
        <Card key={i} className="border bg-white/80">
  ...
  const StudentsSkeleton = () => (
    <div className="grid gap-2">
      {[...Array(4)].map((_, i) => (
        <Card key={i} className="border bg-white/80">
          <CardContent className="p-3 sm:p-4 animate-pulse space-y-2">
  ...
  if (isLoading) {
    return (
      <DashboardLayout>
        <div className="p-3 sm:p-4 md:p-6 space-y-4 max-w-6xl mx-auto">
          <StatsSkeleton />
          <StudentsSkeleton />
        </div>
      </DashboardLayout>
      ...
      <div className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-5 max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="relative flex-1 max-w-xs">
              <Search className="h-4 w-4 text-muted-foreground absolute top-2.5 start-2" />
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder={isAr ? "ابحث بالاسم" : "Search by name"}
                className="w-full rounded-lg border border-orange-100 bg-white/80 ps-8 pe-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-orange-200"
              />
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        {filteredStudents.length === 0 ? (
          ...
          <div className={`grid gap-2 ${compact ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}> 
            {filteredStudents.map(({ student, latest, status, surveyCount }, i) => (
              <motion.div
                key={student.id}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
